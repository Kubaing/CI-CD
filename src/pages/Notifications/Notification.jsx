              ...
              <table>
                ...
                <tbody>
                  {leaveRequests.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan="7">ไม่มีข้อมูลคำขอลางาน</td>
                    </tr>
                  )}
                </tbody>
              </table>
                  ...
                  {workSchedules.length > 0 ? (
                    workSchedules.map((item, index) => (
                      <tr key={item._id}>
                        <td>{index + 1}</td>
                        <td>{item.username}</td>
                        <td>{item.task || "ไม่ระบุประเภทงาน"}</td>
                        <td>
                          {new Date(item.date).toLocaleDateString("th-TH")}
                        </td>
                        <td>{item.hours || "-"}</td>
                          ...
                          {item.status === "Pending" && (
                            <>
                              <div className="action-buttons">
                                <button
                                  className="btn btn-primary"
                                  onClick={() => handleApproveWork(item._id)}
                                >
                                  อนุมัติ
                                </button>
                                <button
                                  className="btn btn-danger"
                                  onClick={() => handleRejectWork(item._id)}
                                >
                                  ปฏิเสธ
                                </button>
                              </div>
                    ...
                    <tr>
                      <td colSpan="7">ไม่มีข้อมูลการลงงาน</td>
                    </tr>
                  )}
                </tbody>
              </table>
